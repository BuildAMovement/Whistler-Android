...
abstract class CipherOpenHelper extends SQLiteOpenHelper {
    ...
    private volatile CacheWordHandler mHandler;
    private CacheWordHandler sqlHandler;

    CipherOpenHelper(CacheWordHandler cacheWord, Context context, String name,
                     CursorFactory factory, int version) {
        super(context, name, factory, version, new CipherV3MigrationHook());
        mHandler = cacheWord;

        sqlHandler = new CacheWordHandler(context, new SQLCacheWordSubscriber());
        sqlHandler.connectToService();
    }
    ...
    private class SQLCacheWordSubscriber implements ICacheWordSubscriber {
        @Override
        public void onCacheWordUninitialized() {
            Timber.d("***** onCacheWordUninitialized");
        }

        @Override
        public void onCacheWordLocked() {
            Timber.d("***** onCacheWordLocked");
        }

        @Override
        public void onCacheWordOpened() {
            Timber.d("***** onCacheWordOpened");
            mHandler = sqlHandler;
        }
    }
}
